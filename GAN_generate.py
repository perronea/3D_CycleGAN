...
class CycleGAN():
    def __init__(self, image_folder, input_modality, gen_mod, 
                 lr_D=2e-4, lr_G=2e-3, #image_shape=(204, 292, 1), #image_shape=(304, 256, 1),
                 date_time_string_addition=''):
        #self.img_shape = image_shape
        self.channels = 1 #self.img_shape[-1]
        ...
        data = load_slices.load_slices(image_folder, input_modality,
        ...
        self.C_images = data["C_images"]
        self.S_images = data["S_images"]
        self.T_images = data["T_images"]
        self.C_image_names = data["C_image_names"]
        self.S_image_names = data["S_image_names"]
        self.T_image_names = data["T_image_names"]
...
#         self.load_model_and_generate_synthetic_images()
        ...
        self.load_model_and_generate_synthetic_images(image_folder, gen_mod)
...
# Save and load
    ...
    def load_model_and_weights(self, model):
        path_to_model = os.path.join('models', '{}.json'.format(model.name))
        path_to_weights = os.path.join('models', '{}.hdf5'.format(model.name))
        #model = model_from_json(path_to_model)
        model.load_weights(path_to_weights)

    def load_model_and_generate_synthetic_images(self, image_folder, gen_mod):
        #response = input('Are you sure you want to generate synthetic images instead of training? (y/n): ')[0].lower()
        #if response == 'y':
        def save_image(image, name, gen_mod, axis):
            if self.channels == 1:
                image = image[:, :, 0]
            synthetic_image_dir = os.path.join(image_folder, 'synthetic_images', gen_mod, axis)
            os.makedirs(synthetic_image_dir, exist_ok=True)
            toimage(image, cmin=-1, cmax=1).save(os.path.join(synthetic_image_dir, name))

        if gen_mod == 'T2w':
            self.load_model_and_weights(self.G_A2B)
            synthetic_images_C = self.G_A2B.predict(self.C_images)
            synthetic_images_S = self.G_A2B.predict(self.S_images)
            synthetic_images_T = self.G_A2B.predict(self.T_images)
        elif gen_mod == 'T1w':
            self.load_model_and_weights(self.G_B2A)
            synthetic_images_C = self.G_B2A.predict(self.C_images)
            synthetic_images_S = self.G_B2A.predict(self.S_images)
            synthetic_images_T = self.G_B2A.predict(self.T_images)
        else:
            print('ERROR: Cannot generate the requested modality. "{}" not recognized. Input either "T1w" or "T2w"'.format(gen_mod))
            exit()
                    
        for i in range(len(synthetic_images_C)):
            # Get the name from the image it was conditioned on
            name = self.C_image_names[i].strip('.png') + '_synthetic.png'
            synt_C = synthetic_images_C[i]
            save_image(synt_C, name, gen_mod, 'C')
        for i in range(len(synthetic_images_S)):
            # Get the name from the image it was conditioned on
            name = self.S_image_names[i].strip('.png') + '_synthetic.png'
            synt_S = synthetic_images_S[i]
            save_image(synt_S, name, gen_mod, 'S')
        for i in range(len(synthetic_images_T)):
            # Get the name from the image it was conditioned on
            name = self.T_image_names[i].strip('.png') + '_synthetic.png'
            synt_T = synthetic_images_T[i]
            save_image(synt_T, name, gen_mod, 'T')
        
        print('{} synthetic images have been generated and placed in {}'.format(gen_mod, os.path.join(image_folder, 'synthetic_images', gen_mod)))
